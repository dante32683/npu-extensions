...
- **Resident watcher (`OrganizeKeeper.exe`):** Phase 2 BCL-only companion. argv: `watch [--support-dir <path>]` (default), `status`, `process-one <imagePath>`, `parity-check`. **State at `%LocalAppData%\NpuOrganize\`** — `config.json` (written by Raycast _Start Screenshot Watcher_ from current prefs; **hot-reloaded** when its mtime changes), `state.json` (counters + heartbeat + last cursor), `stop.flag` (sentinel; written by Raycast _Stop_, polled @1Hz), `organize.log` (append-only audit trail). The keeper spawns the sibling **`NpuBridge.exe screenshot-title --ensure-ready`** for each debounced event using the same `cwd = dirname(exe)` rule as Raycast.
- **Battery skip:** keeper-only; `GetSystemPowerStatus` Win32 P/Invoke (`PowerStatus.IsOnAcPower` treats Unknown as AC). Manual Raycast commands ignore this and run unconditionally.
- **Debounce contract:** per-path quiet timer; FSW `Created/Changed/Renamed` events reset it. After `debounceMs` (default 1500) the keeper additionally tries to open the file with exclusive read for up to 5s before processing — this defeats Snipping Tool / Game Bar holding the handle briefly after Created.
- **Slug parity is mandatory:** if the C# `SlugGenerator` (keeper) and the TS `slugify()` (manual Raycast command) ever produce different filenames for the same description, both entrypoints would step on each other on the cursor. `OrganizeKeeper.exe parity-check` runs the same fixtures as `slug.test.ts` (28 cases, hardcoded canonical FNV-1a values for `""`, `"a"`, `"foobar"`) and exits 1 on any mismatch — run it after every keeper publish.
