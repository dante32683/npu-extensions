...
| `npu-dev-toolbox-ext` | Raycast + Phi bridge | `assets/bin/NpuBridge.exe` | `NpuDevToolboxBridge.Identity` | `cwd-of-pid <pid>`, `phi-commit <tempInputFile>` (Raycast cmds: `open-workspace`, `commit-message`) | Active |
| `npu-organize-ext` | Raycast + WinRT bridge + resident keeper | `assets/bin/NpuBridge.exe` + `assets/bin/OrganizeKeeper.exe` | `NpuOrganizeBridge.Identity` (keeper has no identity) | Bridge: `screenshot-title <imagePath> [--ensure-ready] [--no-ocr]` → `{ status, description, confidence, ocrExcerpt, elapsedMs }`. Keeper argv: `watch [--support-dir <path>]`, `status`, `process-one <path>`, `parity-check`. Raycast cmds: `rename-new-screenshots`, `dry-run-screenshot-rename`, `screenshot-watcher`. State at `%LocalAppData%\NpuOrganize\` (`config.json`, `state.json`, `stop.flag`, `organize.log`). | Active (Roadmap §6 phases 0–2) |
